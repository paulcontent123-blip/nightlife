...
export async function EventCard({ event }: { event: Event }) {
    ...
    return (
        <Link
            ...
            prefetch={false}
            className="flex min-w-0 overflow-hidden rounded-xl border border-border bg-void-3 shadow-[0_4px_28px_rgba(0,0,0,.5)] transition-all hover:-translate-y-1 hover:border-amber-border hover:bg-void-4"
        >
            ...
            <div className="min-w-0 flex-1 p-4">
                <p className="mb-1 break-words font-display text-[15px] font-bold leading-snug text-white">{event.title}</p>
                ...
                <div className="flex items-center justify-between">
                    ...
                    <span className="font-display text-[11.5px] font-bold text-amber">
                        {event.is_free ? commonT("free") : eventsT("viewTickets")}
                    </span>
                </div>
            </div>
        </Link>
