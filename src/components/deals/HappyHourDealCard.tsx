...
export async function HappyHourDealCard({ deal, index }: { deal: PublicDeal; index: number }) {
    ...
    return (
        <Link
            ...
            prefetch={false}
            className="block min-w-0 overflow-hidden rounded-xl border border-border bg-void-2 transition-colors hover:border-amber-border hover:bg-void-3"
        >
            <div className={["h-1 bg-gradient-to-r", accent].join(" ")} />
            <div className="p-4">
                <div className="mb-2 flex items-center justify-between gap-2">
                    <p className="min-w-0 truncate text-[10.5px] font-bold uppercase text-amber">{deal.venue.name}</p>
                    {deal.is_exclusive && <Badge tone="amber">VIP</Badge>}
                </div>
                <p className="mb-1 break-words font-display text-base font-bold text-white">{deal.title}</p>
                ...
                <div className="flex items-center justify-between">
                    <span className="text-[11.5px] font-semibold text-cyan">
                        ...
                        {deal.is_open_now && <span className="ml-1.5 text-emerald-400">· {t("happeningNow")}</span>}
                    </span>
